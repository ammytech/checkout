...
(function($) {
			...
			function(e){
				if(e.keyCode!=37 && e.keyCode!=38 && e.keyCode!=39 && e.keyCode!=40 && e.keyCode!=13){
					if($(this).val().length>=minChars){
						$("#spinner").show();
						$.ajax({
							url:me.callBackUrl + encodeURI($(this).val()+(parseInt($("#sby").val())>0?'&sby='+$("#sby").val():'')),
												...
												target.click(function(e){
													if(me.appendText==0){
													   me.textField.val($(this).find(".suggestion_title").text());
														...
														me.textField.val("");
														alreadytext = $("#append-field").val();
														if(alreadytext.length>3){
															newtext = $(this).find(".suggestion_title").text();
															appendtext = alreadytext+','+newtext;
														    $("#append-field").val(appendtext);
														}else{
															$("#append-field").val($(this).find(".suggestion_title").text());
														}
	...
	$.fn.coolautosuggest = function(options) {
		...
		return this.each(function() {
			...
			if(settings.appendText==1){
				obj.appendText=settings.appendText;
			}
